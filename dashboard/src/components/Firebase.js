...
class Firebase {
  constructor() {
    firebase.initializeApp(dashboardConfig.firebaseConfig);
  ...
  getDashboardData = (callback) => {
    ...
    const unsubscriber = ref.onSnapshot((snap) => {
      ...
      const retData = {
        initials: initials,
        name: `${d.firstName.charAt(0).toUpperCase() + d.firstName.slice(1)} ${
          d.lastName.charAt(0).toUpperCase() + d.lastName.slice(1)
        }`,
        email: d.email,
        code: d.code,
        accepted: d.accepted,
        ...
        applicationType:
          d.applicationType.charAt(0).toUpperCase() +
          d.applicationType.slice(1),
      };
      callback(retData);
    });
    return unsubscriber;
